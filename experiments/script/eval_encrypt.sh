...
echo '[*] Running packed encryption experiment on table: '$TABLE | tee $OUT_FILE
...
for gid in $group_ids; do
  if [[ $TABLE == 'tbl_bitcoin' ]]; then
    mysql -u $MYSQL_USER -D $MYSQL_DB -e 'SELECT HERMES_PACK_CONVERT(FLOOR(value / 24)) FROM '$TABLE' WHERE group_id = '$gid';' > /dev/null
  else
    mysql -u $MYSQL_USER -D $MYSQL_DB -e 'SELECT HERMES_PACK_CONVERT(value) FROM '$TABLE' WHERE group_id = '$gid';' > /dev/null
  fi
done
...
echo 'Packed Encrypt:   '$elapsed_pack' ms (avg: '$((elapsed_pack * 1000 / total_rows))' us/row)' | tee -a $OUT_FILE
