...
% Title portion
\title{Hermes: Towards Bridging Relational and Algebraic Abstractions in Homomorphically Encrypted Databases}
...
\begin{itemize}
    \item We formalize a vectorized ciphertext encoding that embeds precomputed aggregate statistics directly into the cryptographic layout. This architectural innovation reduces homomorphic aggregation from scaling logarithmically with Galois rotations to simple constant-time additions, completely amortizing the overall encryption overhead.

    \item We develop data-oblivious shift and mask algorithms to achieve in-place, slot-level insertions and deletions. This enables dynamic tuple mutability over packed ciphertexts while preserving strict semantic security and avoiding conventional client-side recomputations and interactions.

    \item We implement Hermes natively as MySQL loadable functions to execute homomorphic query pipelines entirely within the DBMS, marking, to the best of our knowledge, the first practical integration of fully homomorphic encryption into an industrial-grade relational database engine. 
\end{itemize}

Our extensive evaluations are conducted across diverse real-world datasets, encompassing genomic annotations, cryptocurrency trade volumes, and healthcare statistics. We comprehensively benchmark the system against four distinct execution baselines: unencrypted plaintext SQL, conventional singular FHE, a standard packed FHE configuration relying on logarithmic Galois rotations~\cite{openfhe}, and the Intel Paillier Cryptosystem Library~\cite{ipcl_github}. Extensive evaluations demonstrate significant performance improvements, including an over 3400x increase in encryption throughput, a 4000x speedup for tuple insertions, and a 300x acceleration for deletions when compared to conventional scalar FHE deployments.
...
This paper presented Hermes, a fully homomorphic encrypted database subsystem that bridges the structural divide between relational data models and algebraic cryptographic circuits. By introducing a vectorized packing strategy with embedded auxiliary statistics, Hermes transforms homomorphic aggregation from an operation scaling logarithmically with rotations into a constant-time addition. Furthermore, the development of data-oblivious masking and shifting algorithms enables secure in-place modifications, completely eliminating the need for client-side decryption or re-encryption during routine database maintenance.

% The core contribution of this work extends beyond algorithmic optimizations to demonstrate the first practical integration of Fully Homomorphic Encryption within an industrial grade relational database engine. Rather than relying on isolated research prototypes or proxy architectures, Hermes is implemented natively as MySQL loadable functions. Our extensive evaluation against production-ready cryptographic standards, specifically OpenFHE and the Intel Paillier Cryptosystem Library, establishes a new bar for encrypted systems. The empirical results reveal transformative performance enhancements, including an over 3400x improvement in encryption throughput, alongside staggering speedups of over 4000x for tuple insertions and 300x for deletions. These extreme performance gains, achieving sub millisecond amortized latencies, elevate fully homomorphic encryption from theoretical feasibility to practical deployability for real-world cloud infrastructure.

While Hermes currently focuses on single-attribute aggregation and point mutability, the underlying vectorized architecture is designed to be extensible. Our future work includes implementing multi-column ciphertext layouts and homomorphic join operators by leveraging the established SIMD infrastructure to support the full range of relational algebra.
