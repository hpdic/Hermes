...
We present \emph{Hermes}, a prototype system that enables secure and efficient vectorized relational query execution over homomorphically encrypted data. Built atop the multi-slot capabilities of modern FHE schemes, Hermes is implemented as a suite of native MySQL loadable functions interfacing with OpenFHE~\cite{openfhe}. Hermes aims to bridge the gap between relational and cryptographic abstractions by introducing three key innovations:

\paragraph{(1) Packed Relational Data Model with Aggregates}
Hermes adopts a packed ciphertext representation that maps multiple database tuples into parallel homomorphic slots, which is a standard practice in most FHE schemes. In addition to that, to accelerate homomorphic aggregation, Hermes reserves a specific slot within each ciphertext to store a precomputed auxiliary value (e.g., a sum) over the data slots. This value is computed at packing time and maintained incrementally. By embedding this analytic state directly into the physical layout, encrypted aggregation queries such as \texttt{SUM} can be answered using constant-time ciphertext additions rather than expensive cryptographic rotations (i.e., Galois automorphisms). This layout amortizes FHE's computational overhead and improves both query throughput and storage efficiency.
We elaborate on the design of this data model in Section~\ref{sec:datamodel}.
...
% As of April 2026, the codebase comprises 4761 lines of code. All components are implemented in C++ and compiled into dynamic libraries registered as MySQL loadable functions. Our minimalist integration strategy avoids any modification to the MySQL source code or query engine internals. The full source code is publicly available on Github.
% % \[
...
% \subsection{Result Extraction and Decryption Interface}

% In a production deployment, Hermes strictly adheres to the standard threat model of fully homomorphic encryption, ensuring that the secret key resides exclusively on the trusted client. The MySQL server is only responsible for evaluating the queries and returning either the memory pointers or the serialized payloads of the resulting ciphertexts. The client application then performs the decryption locally. This architecture guarantees that the database engine never has access to the plaintext records or the secret key.

% However, to facilitate rigorous performance profiling and isolate the cryptographic latency from network overhead, Hermes provides a localized decryption harness strictly for benchmarking purposes. This evaluation interface operates entirely within the MySQL server process and is exposed via dedicated read only UDFs. The execution workflow initiates when the SQL engine passes a hexadecimal string handle to the decryption function. The C++ plugin parses this string to resolve the memory address and performs strict pointer validation to confirm alignment with the expected OpenFHE \texttt{Ciphertext<DCRTPoly>} memory layout.

% When operating in this testing mode, the plugin invokes the OpenFHE \texttt{Decrypt} API using a resident evaluation key provisioned solely for experimental environments. The decrypted output is subsequently converted into a standard C++ vector using \texttt{SetLength} and \texttt{GetPackedValue}. To minimize memory copying and adhere to the strict scalar return requirements of the MySQL FFI, the plugin logic extracts only the specifically requested integer slot, avoiding the construction of oversized response buffers.
