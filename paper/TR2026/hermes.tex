...
% Title portion
\title{Hermes: Homomorphically Encrypted Vector Databases without Galois Rotations}
...
\begin{abstract}
Fully Homomorphic Encryption (FHE) has long promised the ability to compute over encrypted data without revealing sensitive contents.
Yet, despite decades of cryptographic advances, practical integration of FHE into real-world databases remains elusive due to the prohibitive overhead (both time and space) and the non-arithmetic nature of many database operations. 
This paper presents \emph{Hermes}, the first system supporting FHE-native vector queries inside a standard SQL engine. By leveraging the multi-slot capabilities of modern FHE schemes, Hermes introduces a new data model that packs multiple records per ciphertext and embeds encrypted aggregate data (e.g., local sums) to support efficient in-place updates. 
To overcome ciphertext immutability with record-level modifications, we develop new homomorphic algorithms based on slot masking, shifting, and rewriting. 
Hermes has been implemented as native C++ loadable functions in MySQL using OpenFHE v1.2.4, comprising over 3,500 lines of code. 
Experiments on real-world datasets demonstrate up to 1{,}600$\times$ higher throughput in encryption and over 30$\times$ speedup in insertion compared to native FHE implementations.
\end{abstract}
...
\keywords{Vector databases, Fully homomorphic encryption, User-defined functions}
...
\don{This is an excellent introduction on FHE+DB, but it needs revision: We want to focus on vector databases, not general (realtaional) databases. I agree FHE+DB is an important problem for sure; but it's not the claim of this paper. Maybe this implies we need to do more experiments on vector data.}
...
Among all cryptographic primitives, Fully Homomorphic Encryption (FHE) stands out for its conceptual power: it allows algebraic operations on encrypted data, thereby eliminating the need to trust the compute server. Since Gentry’s seminal construction~\cite{gentry2009}, subsequent schemes such as BFV and CKKS~\cite{FV2012,ckks2016} have made dramatic advances in efficiency, expressiveness, and implementation maturity~\cite{openfhe}. Today, FHE is no longer a theoretical curiosity; it underpins practical tools in secure machine learning and privacy-preserving analytics~\cite{boemer2020mp2ml,han2023}.

And yet, one foundational system remains conspicuously absent from this evolution: Databases. 
Despite decades of research on encrypted databases~\cite{popa2011cryptdb,arx_vldb19}, all practical systems either leak access patterns through auxiliary indexing or offload all computation to the client. The longstanding vision of a relational engine that operates directly on semantically secure ciphertexts remains, as of today, unrealized.
This disconnect is not for lack of demand or cryptographic capability, but arises from a deeper structural asymmetry: the absence of a principled interface between database semantics and homomorphic computation. Traditional DBMSs operate over mutable records, evolving schemas, and runtime query plans. FHE schemes, by contrast, operate over static circuits and immutable ciphertexts. 

Bridging this divide requires more than encrypting inputs or wrapping FHE inside SQL. It demands a new class of system architecture that reconciles the structural invariants of encrypted computation with the operational dynamics of relational execution. Without such an alignment, even the most powerful cryptographic schemes remain impractical for secure query processing.

As we stand at the convergence of deployable FHE toolchains and rising expectations for secure cloud analytics, the foundational question is no longer \emph{whether} relational database systems should support native homomorphic execution, but \emph{how}, and with what interface. This work, thus, is a principled step toward answering that question.
...
\begin{itemize}
    \item \textit{SIMD-aware relational data model with embedded analytics.} We propose a vectorized data model where each homomorphic ciphertext stores multiple database tuples in parallel slots. By reserving a specific slot for a precomputed auxiliary local sum, this model supports efficient encrypted aggregation without the costly Galois rotations. As a result, this new data model amortizes encryption overhead and reduces per-tuple processing cost.

    \item \textit{Data oblivious mutability primitives.} We design homomorphic primitives for slot-level insertion and deletion using encrypted shift and mask operations. These primitives allow structural updates to packed ciphertexts without decryption or full reencryption, supporting secure and dynamic record-level mutability.

    \item \textit{FHE-native query engine integration.} We implement the above data model and primitives in a system prototype, namely Hermes, as a collection of C++ loadable functions interfacing OpenFHE with MySQL UDF. The system executes homomorphically encrypted exectuion pipelines inside the DBMS without external interaction.
\end{itemize}
