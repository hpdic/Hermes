...
# ==============================================================================
ax1.plot(scales, time_enc_hg38, marker=markers[0], color=colors[0], label=datasets[0], linewidth=3, markersize=12)
ax1.plot(scales, time_enc_bitcoin, marker=markers[1], color=colors[1], label=datasets[1], linewidth=3, markersize=12)
ax1.plot(scales, time_enc_covid19, marker=markers[2], color=colors[2], label=datasets[2], linewidth=3, markersize=12)
...
ax1.set_xticklabels([str(s) for s in scales], fontsize=20, rotation=15)
ax1.tick_params(axis='y', labelsize=20)

ax1.set_xlabel('Packing Scale (slots)', fontsize=24)
ax1.set_ylabel('Total Encryption Time (ms)', fontsize=24)
ax1.set_title('(a) Encryption Scalability', fontsize=28, pad=20)
...
ax2.set_ylabel('Insertion Time for 100 ops (ms)', fontsize=24)
ax2.set_title('(b) Insertion Performance', fontsize=28, pad=20)

# 设置 X 轴类别标签
ax2.set_xticks(x_indices)
ax2.set_xticklabels([str(s) for s in scales], fontsize=20, rotation=15)
ax2.tick_params(axis='y', labelsize=20)
ax2.set_xlabel('Packing Scale (slots)', fontsize=24)

# Y 轴线性刻度，设置合适上限以突出平稳性并给图例留出空间
...
ax3.set_ylabel('Deletion Time for 100 ops (ms)', fontsize=24)
ax3.set_title('(c) Deletion Performance', fontsize=28, pad=20)

# 设置 X 轴类别标签
ax3.set_xticks(x_indices)
ax3.set_xticklabels([str(s) for s in scales], fontsize=20, rotation=15)
ax3.tick_params(axis='y', labelsize=20)
ax3.set_xlabel('Packing Scale (slots)', fontsize=24)
...
print('Giant font scalability plot saved to: ' + output_path)